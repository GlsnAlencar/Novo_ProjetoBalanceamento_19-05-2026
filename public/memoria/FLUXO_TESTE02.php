...
<head>
    ...
    <title>Fluxo Teste02 - CRUD Local com Correções</title>
    ...
    <style>
        * {
        ...
        body {
            ...
            height: 100vh; /* Força altura total para evitar scroll de body */
            overflow: hidden;
        }
...
<body>
    ...
    <script>
        ...
        var isLoading = false;
        var isDirty = false;
        var saveTimeout = null; // Para debounce de salvamento
        var lastNodeYPosition = {}; // Rastreia última posição Y por pai para evitar sobreposição
                ...
                if (currentLinhaId) {
                    ...
                    if (data.status === 'success' && data.drawflow_data) {
                        ...
                        if (flowData?.drawflow?.Home?.data && Object.keys(flowData.drawflow.Home.data).length > 0) {
                            // ✅ CORREÇÃO 1: Clear antes de import para evitar mistura de estados
                            editor.clear();
                            mockPostos = [];
                            mockConnections = [];
                            lastNodeYPosition = {};
                            ...
                            // ✅ CORREÇÃO 2: Reconstruir mockPostos do flowData importado
                            const homeData = flowData.drawflow.Home.data;
                            Object.keys(homeData).forEach(dfId => {
                                const node = homeData[dfId];
                                const nodeType = node.data?.type || node.class || 'node';
                                
                                mockPostos.push({
                                    id: node.data?.id || 'unknown_' + dfId,
                                    name: node.data?.name || 'Nó sem nome',
                                    type: nodeType,
                                    tc: node.data?.tc || 0,
                                    drawflow_id: parseInt(dfId),
                                    x: parseFloat(node.pos_x) || 100,
                                    y: parseFloat(node.pos_y) || 150
                                });

                                // ✅ ETAPA 1: Hidratação do lastNodeYPosition
                                // Se o nó tiver um input vindo de algum lugar, tentamos identificar o pai
                                // para que novos nós criados não sobreponham os antigos
                                if (node.inputs && node.inputs.input_1) {
                                    node.inputs.input_1.connections.forEach(conn => {
                                        const parentDfId = conn.node;
                                        const parentNode = homeData[parentDfId];
                                        if (parentNode && parentNode.data && parentNode.data.id) {
                                            const pId = parentNode.data.id;
                                            if (!lastNodeYPosition[pId]) lastNodeYPosition[pId] = { count: 0 };
                                            lastNodeYPosition[pId].count++;
                                        }
                                    });
                                }

                                // ✅ CORREÇÃO: Re-aplicar classes BPMN perdidas
                                const nodeEl = document.getElementById('node-' + dfId);
                                if (nodeEl) {
                                    if (nodeType === 'decision_exclusive') nodeEl.classList.add('decision_exclusive');
                                    else if (nodeType === 'decision_parallel') nodeEl.classList.add('decision_parallel');
                                }
                            });

                            // ✅ CORREÇÃO 3: Reconstruir mockConnections para integridade do estado JS
                            Object.keys(homeData).forEach(dfId => {
                                const node = homeData[dfId];
                                if (node.outputs && node.outputs.output_1) {
                                    node.outputs.output_1.connections.forEach(conn => {
                                        mockConnections.push({
                                            sourceDfId: parseInt(dfId),
                                            targetDfId: parseInt(conn.node)
                                        });
                                    });
                                }
                            });
        ...
        async function addNodeToFlow(name = null, type = 'node', parentId = null, initialX = null, initialY = null) {
            ...
            if (parentId) {
                ...
                if (parentPosto) {
                    ...
                    // ✅ CORREÇÃO: Incrementar Y automaticamente para evitar sobreposição
                    if (!lastNodeYPosition[parentId]) {
                        lastNodeYPosition[parentId] = { count: 0 };
                    }
                    // Ajuste de espaçamento: 120px de respiro vertical entre irmãos
                    const incrementoY = lastNodeYPosition[parentId].count * 120;
                    // O primeiro nó derivado já deve ter um offset do pai (ex: +100)
                    suggestedY = (parentPosto.y || 150) + (lastNodeYPosition[parentId].count === 0 ? 0 : incrementoY);
                    lastNodeYPosition[parentId].count++;
                }
            ...
            try {
                ...
                if (data.status === 'success') {
                    ...
                    const drawflowId = editor.addNode(
                        ...
                        { id: uniqueId, name: nodeName, type: type, tc: type === 'node' ? tc : 0 },
                        htmlContent
                    );
        ...
        async function editNodeInFlow(id) {
            ...
            if (posto) {
                ...
                if (newName && newName.trim()) {
                    ...
                    // ✅ CORREÇÃO 1: Atualizar dados internos do Drawflow com tipo preservado
                    ...
                    // ✅ CORREÇÃO 2: Regenerar HTML completo do nó para refletir mudanças visuais
                    const newHtmlContent = generateNodeHtml({
                        id: posto.id,
                        name: posto.name,
                        type: posto.type,
                        tc: posto.tc
        ...
        function removeNodeFromFlow(id) {
            if (confirm('Tem certeza que deseja remover este posto?')) {
                ...
                if (posto?.drawflow_id) {
                    ...
                    mockPostos = mockPostos.filter(p => p.id != id); // ✅ Comparação flexível
        ...
        function saveFlowState() {
            ...
            // ✅ CORREÇÃO: Implementação de Debounce para evitar excesso de requisições
        ...
        function setupDrawflowEvents() {
            ...
            editor.on('nodeSelected', function(id) {
                ...
                mockPostos.forEach(p => { if (p.drawflow_id != id) removeDeriveOptions(p.drawflow_id); });
            });
